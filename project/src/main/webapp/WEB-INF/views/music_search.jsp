...
<main id="main">
    <div class="contents_area">
        <h2 class="maintitle">검색결과</h2>
        <div class="nation_box">
            <div class="internal${nationChk1}">
                <a href="/music/musicSearch?search_word=${param.search_word}&category=0">국내</a>
            </div>
            <div class="external${nationChk2}">
                <a href="/music/musicSearch?search_word=${param.search_word}&category=1">국외</a>
            </div>
        </div>
        <div class="music_search_wrap">
            <h3 class="subtitle">제목별</h3>
            <table class="music_search_main">
                <colgroup>
                    <col style="width: 8%;">
                    <col style="width: 8%;">
                    <col style="width: 56%;">
                ...
                <tr class="music_search_main_head">
                    <th>년도</th>
                    <th>순위</th>
                    <th>곡정보</th>
                    <th>듣기</th>
                ...
                <c:choose>
                    <c:when test="${empty music_list}">
                        <tr class="music_search_main_row">
                            <th colspan="7" class="result_none">검색결과 없음</th>
                        </tr>
                    </c:when>
                    <c:otherwise>
            ...
            <table class="music_search_main">
                ...
                <c:choose>
                    <c:when test="${empty musician_list}">
                        <tr class="music_search_main_row">
                            <th colspan="7" class="result_none">검색결과 없음</th>
                        </tr>
                    </c:when>
                    <c:otherwise>
                        <c:forEach var="musicDto" items="${musician_list}">
                            <c:set var="bookmark" value="${empty musicDto.mb_no?1:2}"/>
                            <c:set var="likes" value="${empty musicDto.l_no?1:2}"/>
                            <c:set var="likes_totalCnt" value="${empty musicDto.l_totalCnt?0:musicDto.l_totalCnt}"/>
                            <tr class="music_search_main_row">
            ...
            <table class="music_search_main">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <c:forEach var="musicDto" items="${album_list}">
                            <c:set var="bookmark" value="${empty musicDto.mb_no?1:2}"/>
                            <c:set var="likes" value="${empty musicDto.l_no?1:2}"/>
                            <c:set var="likes_totalCnt" value="${empty musicDto.l_totalCnt?0:musicDto.l_totalCnt}"/>
                            <tr class="music_search_main_row">
